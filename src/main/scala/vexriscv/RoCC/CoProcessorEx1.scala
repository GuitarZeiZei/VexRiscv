/**
  ...
  * Instantiation of a Float Cal coprocessor
  */
    ...
    def build(controller: EventController): Unit = {
      ...
      val exec = controller.exec event new Area {
        ...
        val add = new Area{
          ...
          val subRS1Sign = RegNext(rs1.sign)
          val subRS2Sign = RegNext(rs2.sign)
          // exponent match
        ...
        val sub = new Area{
          ...
          val subRS1Sign = RegNext(rs1.sign)
          val subRS2Sign = RegNext(rs2.sign + U(1, 1 bits))
          // exponent match
          val expRS1GE = rs1.exp >= rs2.exp
          val expDiff = Reg(UInt(8 bits))
          val expMax = Reg(UInt(8 bits))
