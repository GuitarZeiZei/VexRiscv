...
public:
	...
	virtual void preCycle(){
		if (top->iBus_cmd_valid && top->iBus_cmd_ready) {
			...
			pendings[wPtr] = (top->iBus_cmd_payload_address);
			wPtr = (wPtr + 1) & 0xFF;
	...
	virtual void postCycle(){
		...
		if(rPtr != wPtr && (!ws->iStall || VL_RANDOM_I(7) < 100)){
        	...
        	rPtr = (rPtr + 1) & 0xFF;
			top->iBus_rsp_payload_data = inst_next;
			top->iBus_rsp_valid = 1;
			top->iBus_rsp_payload_error = error_next;
		} else {
		    top->iBus_rsp_payload_data = VL_RANDOM_I(32);
...
public:
	...
	virtual void preCycle(){
		if (top->dBus_cmd_valid && top->dBus_cmd_ready) {
			...
			ws->dBusAccess(top->dBus_cmd_payload_address,top->dBus_cmd_payload_wr,top->dBus_cmd_payload_length,0xF,&data_next,&error_next);
		}
	}
	...
	virtual void postCycle(){
		...
		if(pending && (!ws->dStall || VL_RANDOM_I(7) < 100)){
			pending = false;
			top->dBus_rsp_valid = 1;
			top->dBus_rsp_payload_data = data_next;
...
//			#endif
		...
		//#ifdef CUSTOM_ROCC
			redo(REDO,WorkspaceRegression("custom_rocc").loadHex(string(REGRESSION_PATH) + "../custom/rocc/build/custom_rocc.hex")->bootAt(0x00000000u)->run(50e3););
		//#endif
